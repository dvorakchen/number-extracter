...
import { BinaryImage, FailResp, ImageResult, SelectedImage, SuccessResp } from "./models";
...
function handleRemoveFile(file: SelectedImage) {
  imgList.value = imgList.value.filter(f =>
    f.id !== file.id
  );
}
...
async function handleExtract() {
  ...
  nextTick(async () => {
    const preImages = await Promise.all(imgList.value.map(async img => {
    ...
    resultList.value = new ImageResult(resp.success.map(
      e => new SuccessResp(e.id, e.track_number, imgList.value.find(img => img.id === e.id)!.file)),
      resp.fail.map(e => new FailResp(e, imgList.value.find(img => img.id === e)!.file)))
...
<template>
  <main class="flex p-4">
    <section class="flex flex-col border-r-2 px-4 min-h-ful w-2xl gap-2 relative">
      <div>
        <button class="btn btn-primary" @click="handleSelectImgs" :disabled="loadingImgs">{{ loadingImgs ? '加载图片中' :
          '选择图片' }}</button>
        <input type="file" ref="selectImgs" multiple accept="image/png, image/jpeg" hidden @change="handleInputImgs" />
      </div>
      <div class="flex flex-col gap-2">
        <div class="space-x-2">
          <button class="btn btn-accent" @click="handleExtract" :disabled="!allowExtract">开始提取</button>
          <button class="btn btn-error" @click="clearAll">清除所有</button>
        </div>
        <div class="" v-show="imgList.length">选择了 {{ imgList.length }} 张图片</div>
        <ImgList :list="imgList" @remove="handleRemoveFile" />
      </div>

      <div class="absolute inset-0 bg-gray-400/50 flex flex-col gap-2 items-center justify-center" v-show="extracting">
        <div>提取中
          <span class="loading loading-spinner text-accent"></span>
        </div>
        <!-- <button class="btn" @click="handleCancel">取消提取</button> -->
      </div>

    </section>
    <section class="flex flex-col p-4 w-full h-full">
      <ExtractResult :image-result="resultList" @trigger="handleTrigger" />
    </section>
  </main>
</template>
